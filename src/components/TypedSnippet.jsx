...
import styles from "./TypedSnippet.module.css";
    ...
    <div className={`flex gap-2 mt-5 ${styles.dotsNav}`}>
      {Array.from({ length: snippetCount }).map((_, i) => (
        <button
          ...
          style={{
            background:
              i === current ? "var(--primary, #22c55e)" : "transparent",
...
export default function TypedSnippet({ current, snippetCount, onSelect }) {
    ...
    function tick() {
      ...
      if (state.phase === "typing") {
        ...
        if (state.charIdx === 0 || tok.v[state.charIdx - 1] === "\n") {
          ...
          const spans = typedRef.current.querySelectorAll(
            "[data-current-token]",
          );
